...
const DotField = memo(({
  dotRadius = 2.0, // Aumentado para melhor visibilidade inicial
  dotSpacing = 14, // Mais denso
  cursorRadius = 300,
  cursorForce = 0.1,
  bulgeOnly = true,
  bulgeStrength = 60,
  ...
  sparkle = false,
  waveAmplitude = 0,
  gradientFrom = '#A855F7', // Padrão roxo vibrante
  gradientTo = '#B497CF',
  ...
  const glowId = `dot-field-glow-${stableId.replace(/:/g, '')}`;
  ...
  useEffect(() => {
    ...
    function tick() {
      ...
      // Cálculo de engajamento baseado na velocidade do mouse
      ...
      const crSq = cr * cr;
      const rad = (p.dotRadius as number); // Raio real
      ...
      for (let i = 0; i < len; i++) {
        ...
        if (distSq < crSq) {
          ...
          if (isBulge) {
            const t = 1 - dist / cr;
            const push = t * t * (p.bulgeStrength as number);
